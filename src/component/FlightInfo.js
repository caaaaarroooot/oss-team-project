...
const ArrivalFlights = () => {
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', flexDirection: 'column', border: '1px solid #ccc', padding: '20px', borderRadius: '10px', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.1)', maxWidth: '800px', margin: 'auto' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {flightsToDisplay.map((flight, index) => (
              <tr key={index}>
                <td style={{ borderBottom: '1px solid #ddd', padding: '10px' }}>{flight.arrivalTime}</td>
                <td style={{ borderBottom: '1px solid #ddd', padding: '10px' }}>{flight.airlineCode}</td>
      ...
      <div style={{ marginTop: '20px' }}>
        <button disabled={page === 0} onClick={() => setPage(page - 1)} style={{ marginRight: '10px', padding: '10px', borderRadius: '5px', border: '1px solid #ccc', cursor: 'pointer' }}>
          Previous
        </button>
        <button
